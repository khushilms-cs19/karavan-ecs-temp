...
exports.ComponentApi = {
    setSupportedOnly: function (supportedOnly) {
        SupportedOnly = supportedOnly;
    },
    saveSupportedComponents: function (jsons) {
        SupportedComponents.length = 0;
        var sc = JSON.parse(jsons).map(function (json) { return new ComponentModels_1.SupportedComponent(json); });
        SupportedComponents.push.apply(SupportedComponents, sc);
    },
    ...
    saveComponents: function (jsons, clean) {
        ...
        Components.push.apply(Components, components);
    },
    saveComponent: function (json) {
        var component = exports.ComponentApi.jsonToComponent(json);
        if (Components.findIndex(function (c) { return c.component.name === component.component.name; }) === -1) {
    ...
    getComponents: function () {
        var comps = [];
        if (SupportedOnly) {
            comps.push.apply(comps, Components.filter(function (comp) { return SupportedComponents.findIndex(function (sc) { return sc.name === comp.component.name; }) !== -1; }));
        }
        else {
            comps.push.apply(comps, Components);
        }
        return comps
            .map(function (comp) {
            var sc = SupportedComponents.find(function (sc) { return sc.name === comp.component.name; });
            if (sc !== undefined) {
    ...
    findByName: function (name) {
        return exports.ComponentApi.getComponents().find(function (c) { return c.component.name === name; });
    },
        ...
        if (name) {
            if (name === 'cxf') { // workaround for CXF component start
                ...
                Array.from(map_1.keys()).forEach(function (key, index) {
                    ...
                    var separator = separators_1[index];
                    result_1.push(val);
                    if (separator)
                        result_1.push(separators_1[index]);
                });
                if (result_1.at(result_1.length - 1) === '')
                    return result_1.slice(0, -2).join(''); // remove last colon
