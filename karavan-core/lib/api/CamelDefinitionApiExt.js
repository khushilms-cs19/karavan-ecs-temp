...
var CamelDefinitionApiExt = /** @class */ (function () {
    ...
    CamelDefinitionApiExt.addStepToIntegration = function (integration, step, parentId, position) {
        ...
        if (step.dslName === 'RouteDefinition') {
            ...
            var flows_1 = [];
            (_c = integration.spec.flows) === null || _c === void 0 ? void 0 : _c.filter(function (flow) { return !['RouteConfigurationDefinition', 'RouteDefinition'].includes(flow.dslName); }).forEach(function (bean) { return flows_1.push(bean); });
            ...
            var routeConfigurations = CamelDefinitionApiExt.addStepToSteps(((_e = integration.spec.flows) === null || _e === void 0 ? void 0 : _e.filter(function (flow) { return flow.dslName === 'RouteConfigurationDefinition'; })) || [], step, parentId, position);
            flows_1.push.apply(flows_1, routeConfigurations);
    ...
    CamelDefinitionApiExt.findElementInIntegration = function (integration, uuid) {
        var _b;
        return (_b = _a.findElementMetaInIntegration(integration, uuid)) === null || _b === void 0 ? void 0 : _b.step;
    };
    CamelDefinitionApiExt.findElementMetaInIntegration = function (integration, uuid) {
        var _b;
        var i = CamelUtil_1.CamelUtil.cloneIntegration(integration);
        var routes = (_b = i.spec.flows) === null || _b === void 0 ? void 0 : _b.filter(function (f) { return ['RouteConfigurationDefinition', 'RouteDefinition'].includes(f.dslName); });
        return CamelDefinitionApiExt.findElementInElements(routes, uuid);
    };
    CamelDefinitionApiExt.findElementPathUuids = function (integration, uuid) {
        var _b;
        var result = [];
        var meta = CamelDefinitionApiExt.findElementMetaInIntegration(integration, uuid);
        if (meta && meta.parentUuid) {
            while (((_b = meta.step) === null || _b === void 0 ? void 0 : _b.dslName) !== 'FromDefinition') {
                if (meta.parentUuid) {
    ...
    CamelDefinitionApiExt.findElementInElements = function (steps, uuid, result, parentUuid) {
        ...
        if (steps !== undefined) {
            var _loop_1 = function (index, step) {
                if (step.uuid === uuid) {
                    result = new IntegrationDefinition_1.CamelElementMeta(step, parentUuid, index);
                    return "break";
                }
                else {
        ...
        return new IntegrationDefinition_1.CamelElementMeta(result === null || result === void 0 ? void 0 : result.step, result === null || result === void 0 ? void 0 : result.parentUuid, result === null || result === void 0 ? void 0 : result.position);
    };
    CamelDefinitionApiExt.hasElementWithId = function (integration, id) {
        var hasId = false;
        return CamelDefinitionApiExt.checkIfHasId(integration, id, hasId);
    };
    CamelDefinitionApiExt.checkIfHasId = function (obj, id, hasId) {
    ...
    CamelDefinitionApiExt.moveRouteElement = function (integration, source, target, asChild) {
        var _b, _c, _d, _e, _f;
        var sourceFindStep = CamelDefinitionApiExt.findElementMetaInIntegration(integration, source);
        var sourceStep = sourceFindStep.step;
        var sourceUuid = sourceStep === null || sourceStep === void 0 ? void 0 : sourceStep.uuid;
        var targetFindStep = CamelDefinitionApiExt.findElementMetaInIntegration(integration, target);
        var parentUuid = targetFindStep.parentUuid;
        if (sourceUuid && parentUuid && sourceStep && !_a.findElementPathUuids(integration, target).includes(source)) {
            CamelDefinitionApiExt.deleteStepFromIntegration(integration, sourceUuid);
            if (asChild) {
                return CamelDefinitionApiExt.addStepToIntegration(integration, sourceStep, target, (_c = (_b = targetFindStep === null || targetFindStep === void 0 ? void 0 : targetFindStep.step) === null || _b === void 0 ? void 0 : _b.steps) === null || _c === void 0 ? void 0 : _c.length);
            }
            else {
                switch ((_d = targetFindStep.step) === null || _d === void 0 ? void 0 : _d.dslName) {
                    case 'when':
                    ...
                    case 'otherwise':
                        return CamelDefinitionApiExt.addStepToIntegration(integration, sourceStep, (_f = targetFindStep.step) === null || _f === void 0 ? void 0 : _f.uuid, undefined);
                    default:
    ...
    CamelDefinitionApiExt.deleteStepFromIntegration = function (integration, uuidToDelete) {
        var _b, _c;
        var flows = [];
        (_b = integration.spec.flows) === null || _b === void 0 ? void 0 : _b.filter(function (flow) { return !['RouteConfigurationDefinition', 'RouteDefinition'].includes(flow.dslName); }).forEach(function (x) { return flows.push(x); });
        var routes = CamelDefinitionApiExt.deleteStepFromSteps((_c = integration.spec.flows) === null || _c === void 0 ? void 0 : _c.filter(function (flow) { return ['RouteConfigurationDefinition', 'RouteDefinition'].includes(flow.dslName); }), uuidToDelete);
    ...
    CamelDefinitionApiExt.addRouteConfigurationToIntegration = function (integration, routeConfiguration) {
        var _b;
        (_b = integration.spec.flows) === null || _b === void 0 ? void 0 : _b.push(routeConfiguration);
        return integration;
    };
    CamelDefinitionApiExt.deleteRouteConfigurationFromIntegration = function (integration, routeConfiguration) {
        var _b, _c;
        var flows = [];
        flows.push.apply(flows, ((_b = integration.spec.flows) === null || _b === void 0 ? void 0 : _b.filter(function (flow) { return flow.dslName !== 'RouteConfigurationDefinition'; })) || []);
        flows.push.apply(flows, ((_c = integration.spec.flows) === null || _c === void 0 ? void 0 : _c.filter(function (flow) { return flow.dslName == 'RouteConfigurationDefinition' && flow.uuid !== routeConfiguration.uuid; })) || []);
        integration.spec.flows = flows;
        return integration;
    };
    CamelDefinitionApiExt.updateRouteConfigurationToIntegration = function (integration, e) {
        var _b, _c;
        var elementClone = CamelUtil_1.CamelUtil.cloneStep(e);
        var int = CamelUtil_1.CamelUtil.cloneIntegration(integration);
        var flows = [];
        (_b = integration.spec.flows) === null || _b === void 0 ? void 0 : _b.filter(function (f) { return f.dslName !== 'RouteConfigurationDefinition'; }).forEach(function (f) { return flows.push(f); });
        (_c = integration.spec.flows) === null || _c === void 0 ? void 0 : _c.filter(function (f) { return f.dslName === 'RouteConfigurationDefinition'; }).forEach(function (f) {
            var route = CamelDefinitionApiExt.updateElement(f, elementClone);
            flows.push(CamelDefinitionApi_1.CamelDefinitionApi.createRouteConfigurationDefinition(route));
        });
    ...
    CamelDefinitionApiExt.updateIntegrationRouteElement = function (integration, e) {
        ...
        var int = CamelUtil_1.CamelUtil.cloneIntegration(integration);
        var flows = [];
        (_b = integration.spec.flows) === null || _b === void 0 ? void 0 : _b.filter(function (f) { return !['RouteConfigurationDefinition', 'RouteDefinition'].includes(f.dslName); }).forEach(function (f) { return flows.push(f); });
        (_c = integration.spec.flows) === null || _c === void 0 ? void 0 : _c.filter(function (f) { return f.dslName === 'RouteConfigurationDefinition'; }).forEach(function (f) {
            var routeConfiguration = CamelDefinitionApiExt.updateElement(f, elementClone);
            flows.push(CamelDefinitionApi_1.CamelDefinitionApi.createRouteConfigurationDefinition(routeConfiguration));
        });
        (_d = integration.spec.flows) === null || _d === void 0 ? void 0 : _d.filter(function (f) { return f.dslName === 'RouteDefinition'; }).forEach(function (f) {
            var route = CamelDefinitionApiExt.updateElement(f, elementClone);
    ...
    CamelDefinitionApiExt.getElementProperties = function (className) {
        ...
        if (className) {
            ...
            properties === null || properties === void 0 ? void 0 : properties.filter(function (p) { return p.name !== 'steps'; }).filter(function (p) { return p.name !== 'configurationRef'; }).filter(function (p) { return (className === 'ToDefinition' && p.name !== 'pattern') || className !== 'ToDefinition'; }).forEach(function (p) {
                switch (p.name) {
                    case 'uri':
                        uri = p;
                        break;
