...
import { faMagnifyingGlassPlus, faMagnifyingGlassMinus, faArrowsUpDownLeftRight, faArrowsRotate } from '@fortawesome/free-solid-svg-icons';
...
export class RouteDesigner extends React.Component<Props, RouteDesignerState> {
    ...
    async handleThambnailClick(routeId: number) {  
        await html2canvas(this.sourceRef.current as HTMLDivElement).then((canvas) => {
            const image = canvas.toDataURL();
            const exampleImg = document.getElementById(routeId.toString()) as HTMLImageElement;
            exampleImg.setAttribute("src", image);
            localStorage.setItem(routeId.toString(), image)
        });
    }
    ...
    componentDidUpdate = (prevProps: Readonly<Props>, prevState: Readonly<RouteDesignerState>, snapshot?: any) => {
        if (prevState.activeTabKey !== this.state.activeTabKey) {
            this.handleThambnailClick(this.state.selectedRoutes[this.state.activeTabKey]);
        }  
            ...
            <div ref={this.state.printerRef} className="graph">
                ...
                <div className="flows" data-click="FLOWS" onClick={event => this.state.logic.unselectElement(event)}
                    ...
                    {this.state.routeView !== 'View All' &&
                        <div className='routes sticky' style={{background:'white',zIndex:10000}}>
                            <RoutesTab
                                routes={routes}
                            ...
                            <Flex spaceItems={{ default: 'spaceItemsNone' }} justifyContent={{ default: 'justifyContentFlexEnd' }} style={{ marginTop: '2px', marginRight: '2px', gap: '2px' }}>
                                ...
                                <FlexItem style={{padding: '8px', marginLeft: '16px'}}>
                                    <FontAwesomeIcon icon={faArrowsRotate} 
                                        style={{color: '#0066cc', fontSize: 'large', cursor: 'pointer'}}
                                        onClick={
                                            () => this.handleThambnailClick(this.state.selectedRoutes[this.state.activeTabKey])
                                        }
                                    />
                ...
                <div className='thumbnail-section' style={{position:'sticky'}}>
                    <div className='thumbnail-header' style={{ zIndex: 100, position:'sticky' }} >
                        ...
                        <div className='thumbnail-body scrollable' style={{zIndex: 1}}>
                            {
                                routes.map((route: CamelElement, index: number) => (
                                    <div key={index} className='single-thumbnail' onClick={(event) => {
                                        ...
                                        {localStorage.getItem(index.toString()) === null ?
                                            <img id={index.toString()} className='single-thumbnail-image' src='https://raw.githubusercontent.com/samim-hossain-mondal/karavan-toolkit-icons/main/Screenshot%202023-05-06%20at%201.50.02%20PM.png' alt='' />
                                            :
                                            <img id={index.toString()} className='single-thumbnail-image' src={`${localStorage.getItem(index.toString())}`} alt='' />
                                        }
                                    </div>
                                ))
                            }
        ...
        return (
            <PageSection className="dsl-page" isFilled padding={{ default: 'noPadding' }}>
                <div className="dsl-page-columns">
                    <Drawer isExpanded isInline className='designer-body' style={{height: '90vh'}}>
